...
public class CanvasImpl implements Canvas {
 ...
 // to support advanced operation (redo, undo...)
    ...
    @Override
    public void addModel(Model model) throws InvalidModelException {
        models.add(model);
        if (model instanceof Line) {
            addLine((Line) model);
        } else if (model instanceof Rectangle) {
            addRectangle((Rectangle) model);
        } else if (model instanceof BucketFill) {
            addBucketFill((BucketFill) model);
        }
    }
    ...
    private void addBucketFill(BucketFill bucketFill) {
        if (isOutside(bucketFill.getX(), bucketFill.getY())) {
            throw new InvalidModelException("Error. Bucket fill point is outside of canvas");
        }
    ...
    private void addRectangle(Rectangle rec) {
        if (isOutside(rec.getX1(), rec.getY1())) {
            throw new InvalidModelException("Error. Rectangle is outside of canvas");
        }
    ...
    private void addLine(Line line) {
        if (isOutside(line.getX1(), line.getY1())) {
            throw new InvalidModelException("Error. Line is outside of canvas");
        }
